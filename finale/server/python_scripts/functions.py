...
class App:
    ...
    def return_alignment_nodes(self, list):
        with self.driver.session() as session:
            result = session.read_transaction(self._return_alignment_nodes, list)
            for row in result:
              print(row)
       

    @staticmethod
    def _return_alignment_nodes(tx, list):
        query = (
            "UNWIND $list AS x WITH split(x,',') AS d MATCH (n:node)-[l]-(ni:node) WHERE ((n.ID=toInteger(d[0]) AND ni.ID=toInteger(d[1])) OR (ni.ID=toInteger(d[0]) AND n.ID=toInteger(d[1]))) WITH  n, ni AS nodi CALL { WITH nodi OPTIONAL MATCH (m:metadata) WHERE nodi.sample CONTAINS m.epi_id RETURN [m.epi_id,properties(m)] AS prop} RETURN nodi.ID AS NODO_ID, nodi.sequence AS NODO_sequence, collect(prop) AS sammples"
        )
        result = tx.run(query, list=list)
        return [row for row in result]
    ######
